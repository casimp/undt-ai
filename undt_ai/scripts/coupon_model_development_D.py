...
real-world, experimental test accuracy. Each data instance is re-run 5 time with 
...
been run. Training and testing run on the mean profile thickness.
"""
        ...
        for noise in [0, 0.025, 0.05, 0.1, 0.15, 0.2, 0.3, 0.4, 0.5]:
            ...
            for s_idx, subset in enumerate([None, ]):
                ...
                if not stage_complete:
                    ...
                    x_val_noise, _ = noise_augment(x_val, [y_val_D], levels=[noise])
                    x_test_noise, _ = noise_augment(x_test, [y_test_D], levels=[noise])
                    ...
                    for i in range(5):
                        while True:
                            try:
                                ...
                                # Reset weights
                                reset_weights(model)
                                # Create random noisy data for each training set (ensemble)
                                x_train_noise, _ = noise_augment(x_train, [y_train_D], levels=[noise])
                                ...
                                if run_ID not in df['ID'].values:
                                    ...
                                    # Evaluate
                                    train_rmse = model.evaluate(normal(x_train_noise), [1000 * y_train_D], verbose=0)[1]
                                    val_rmse = model.evaluate(normal(x_val), [1000 * y_val_D], verbose=0)[1]
                                    val_rmse_noise = model.evaluate(normal(x_val_noise), [1000 * y_val_D], verbose=0)[1]
                                    test_rmse = model.evaluate(normal(x_test), [1000 * y_test_D], verbose=0)[1]
                                    test_rmse_noise = model.evaluate(normal(x_test_noise), [1000 * y_test_D], verbose=0)[1]

                                    # Evaluate on coupon data
                                    signal_coup_noise, _ = noise_augment(signal_coup, [D_coup], [noise])
                                    
                                    yhat = model.predict(normal(signal_coup))
                                    yhat_noise = model.predict(normal(signal_coup_noise))

                                    coup_rmse = model.evaluate(normal(signal_coup), [D_coup], verbose=0)[1]
                                    coup_rmse_noise = model.evaluate(normal(signal_coup_noise), [D_coup], verbose=0)[1]
                                ...
                                print('Something went wrong (OSError) - Will retry.')
                                continue
                            break
