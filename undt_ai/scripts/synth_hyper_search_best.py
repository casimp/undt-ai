...
fpath_save = root / 'data/synth_model_matrix_adam_best.pkl'
fpath_ckpnt = root / 'data/checkpoints/ckpnt_best.h5'
fpath_log = root / 'data/checkpoints/training_best.log'
fpath_log_store = root / 'data/synth_model_matrix_adam_log_best.pkl'
...
for idx, values in enumerate(params):
    ...
    for run in range(5):
        ...
        ps_run = dict(zip(keys + ['run'], np.append(values, np.array([run]))))
